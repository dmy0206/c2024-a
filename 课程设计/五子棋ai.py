...
location1,location2,location3 = 0,0,0
...
def negamax(is_ai, depth, alpha, beta):
    ...
    order(blank_list)   # 搜索顺序排序  提高剪枝效率（为了减少思考时间，其实最初是奔着速度去的）
    # 遍历每一个候选步
    for next_step in blank_list:

        global search_count
        search_count += 1
        #出于测试需要定义全局变量search_count
...
def evaluation(is_ai):
    total_score = 0#记录评估局面分数

    #对minmax算法进行了缩减，所以此处进行评估列表的转换
    if is_ai:
        my_list = list1
        enemy_list = list2
    else:
...
def gobangwin():
    win = GraphWin("wuziqi", GRID_WIDTH * (COLUMN+5), GRID_WIDTH * ROW)
    ...
    regretx=16*GRID_WIDTH
    regrety=4*GRID_WIDTH
    l=Line(Point(regretx,regrety), Point(regretx,regrety+2*GRID_WIDTH))
    l.draw(win)
    l=Line(Point(regretx,regrety+2*GRID_WIDTH), Point(regretx+3*GRID_WIDTH,regrety+2*GRID_WIDTH))
    l.draw(win)
    l=Line(Point(regretx,regrety), Point(regretx+3*GRID_WIDTH,regrety))
    l.draw(win)
    l=Line(Point(regretx+3*GRID_WIDTH,regrety), Point(regretx+3*GRID_WIDTH,regrety+2*GRID_WIDTH))
    l.draw(win)
    p1=Point(17.5*GRID_WIDTH,5*GRID_WIDTH)
    string=Text(p1,"悔棋(该功能暂时没用，别点)")
    string.setSize(8)
    string.draw(win)
...
def main():
    ...
    m = 0
    n = 0
    '''piece = Circle(Point(GRID_WIDTH *8, GRID_WIDTH * 8), 16)
    piece.setFill('Black')
    list1.append([8,8])
    piece.draw(win)'''
    ...
    while g == 0:
        ...
        if change % 2 == 1:
            ...
            location1=len(list1)-1
            list1.append(pos)
            location3=len(list3)-1
            list3.append(pos)
            ...
            p2 = win.getMouse()
            #if 16<=round(p2.getX()/GRID_WIDTH<=19) and 4<=round(p2.getY()/GRID_WIDTH<=6):
            #    regret()
